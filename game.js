  ...
  minimap(playerState, units, mapSz) {
    ...
    for(const u of units){
      ...
      if(u.isPlayer){
        ...
        ctx.fillStyle='#00f5ff';ctx.shadowColor='#00f5ff';ctx.shadowBlur=6;ctx.fill();ctx.shadowBlur=0;
      } else if(u.isRemote){
        // Remote human player — diamond shape
        const isEnemy=u.team!==playerTeam;
        const col=isEnemy?'#ff4444':'#00ff88';
        ...
        ctx.restore();
        // Outline for remote players
        ctx.strokeStyle=col;ctx.lineWidth=1;ctx.save();ctx.translate(m.x,m.y);ctx.rotate(Math.PI/4);ctx.beginPath();ctx.rect(-3.5,-3.5,7,7);ctx.stroke();ctx.restore();
      } else {
        // Bot
        ctx.beginPath();ctx.arc(m.x,m.y,3,0,Math.PI*2);
        ctx.fillStyle=u.team==='a'?'#00a8ff':'#ff4400';ctx.fill();
      }
    }
    if(playerState){
...
class NexusStrike {
  ...
  _tickBullets(delta){
    ...
    for(let i=this.bullets.length-1;i>=0;i--){
      ...
      // Add remote players as targets for local bullets
      if(b.ownerId==='local'){
        for(const rt of remoteTargets){
          if(rt.team!==b.ownerTeam) targets.push(rt);
        }
      }
      ...
      for(const t of targets){
        ...
        if(xzDist<bodyRadius && b.position.y>=bodyBase && b.position.y<=bodyTop){
          ...
          if(b.ownerId==='local'){
            ...
            window._dmgQueue.push({pos:b.position.clone(),amount:dmg,t:performance.now(),isRemote:!!t.isRemote,isHeadshot:headshot});
            if(killed){
              this.player.kills++;
              this.hud.killNotif(t.name||'ENEMY');
  ...
  _refreshHUD(){
    ...
    if(this.net?.isConnected){
      for(const id in this.net.players){
        ...
        if(s&&s.alive){
          units.push({position:new THREE.Vector3(s.px||0,s.py||0,s.pz||0),team:s.team||'b',isPlayer:false,isAlive:true,isRemote:true});
        }
      }
    }
  ...
  _tickDamageNumbers(){
    ...
    for(const dq of window._dmgQueue){
      ...
      const color=isHead?'#ffdd00':isRemote?'#ff8800':'#ff4400';
      const glow =isHead?'#ffaa00':isRemote?'#ff6600':'#ff2200';
      const prefix=isHead?'★ ':isRemote?'◈ ':'';
      el.textContent=prefix+dq.amount;
      el.style.cssText=`position:fixed;left:${x}px;top:${y}px;pointer-events:none;z-index:500;font-family:Orbitron,monospace;font-size:${14+Math.min(dq.amount/20,8)}px;color:${color};text-shadow:0 0 8px ${glow};font-weight:900;animation:dmgFloat 0.85s ease forwards;transform:translateX(-50%)`;
