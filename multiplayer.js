...
class NetManager {
  ...
  updateRemoteVisuals(scene) {
    ...
    for (const id in this.players) {
      ...
      const age = Math.min((now - lastUpdate) / 1000, 0.15); // cap extrapolation at 150ms

      // Target position = received pos + velocity extrapolation
      ...
      // Adaptive lerp: faster when far (snapping), smoother when close (polished look)
      const dx = tx - mesh.position.x, dz = tz - mesh.position.z;
      const dist = Math.sqrt(dx*dx + dz*dz);
      const lerpT = dist > 3 ? 0.6 : dist > 1 ? 0.35 : 0.22; // snap if teleporting, smooth if close
